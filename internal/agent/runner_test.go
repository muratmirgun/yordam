	...
	runner := agent.OrchestratedRunner{
		...
		Prepare: func(context.Context, agent.RunInput) (orchestrator.StartTurnRequest, error) {
			return orchestrator.StartTurnRequest{
				Command:      orchestrator.CommandMetadata{CommandID: "command-a", IdempotencyKey: "key-a", RequestDigest: protocol.Digest{Algorithm: protocol.DigestSHA256, Value: strings.Repeat("a", 64)}, Actor: protocol.ActorRef{ID: "user-a", Kind: protocol.ActorUser}},
				ExpectedHead: protocol.CommittedCursor{JournalKind: protocol.JournalSession, JournalID: "session-a", CommitSeq: 1, TransactionID: "head-a"},
				Runtime:      protocol.RuntimeGenerationManifest{ID: "generation-a"},
			}, nil
		},
	}
	input := agent.RunInput{Session: domain.Session{ID: "session-a"}, Prompt: "inspect"}
